...
export default function Toolbar() {
  const { toolbarBgColor, toolbarTextColor, enableSidebarCollapse, enableElementSize, enablePageReload, enableI18n, enableFullscreen, enableColorScheme, enableChangeTheme, enableBreadcrumb, enableAppSetting } = useSysConfigStore(useShallow(state => ({
  ...
  return (
    <ToolbarWrapper className="h-[var(--xt-toolbar-height)] flex items-center px-4" $customToolbarClass={customToolbarClass()}>
      ...
      <div className="ml-auto flex items-center">
        ...
        {enableAppSetting && <AppConfig className="mr-2" />}
        <Personal />
      </div>
    </ToolbarWrapper>
  )
