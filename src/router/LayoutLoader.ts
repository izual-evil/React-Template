...
import { permissionApi } from "@/api/modules/user";
...
function getNoDealMenu() {
  ...
  return (
    cloneRootRoutes[0].children?.find((item) => item.id === "layout")
      ?.children || []
  );
}
...
function generateallSubMenu(noDealMenu: RouteObject[], permissions: string[]) {
  ...
  for (const item of noDealMenu) {
    if (isEmpty(item.meta) && isEmpty(item.children)) continue;
    ...
    if (item.meta?.title && !item.meta.hideInMenu) {
      if (
        item.path === "/" &&
        !useSysConfigStore.getState().app.enableDashboard
      )
        continue;
      if (item.meta?.auth) {
        if (
          (typeof item.meta.auth === "string" &&
            !permissions.includes(item.meta.auth)) ||
          (Array.isArray(item.meta.auth) &&
            !item.meta.auth.some((v) => permissions.includes(v)))
        )
          continue;
      }
      obj.children = [];
...
export default async function LayoutLoader() {
  ...
  if (
    ...
    useSysConfigStore.getState().app.enablePermission
  ) {
